...
def data_from_row(row: WebElement, cell_tag="td", cell_xpath=None) -> List[str]:
    ...
    if cell_xpath:
        return [
            col.text for col in row.find_elements(By.XPATH,cell_xpath)
        ]

    return [
        col.text for col in row.find_elements(By.TAG_NAME,cell_tag)
    ]
...
def table_to_dict(table: WebElement, has_header: bool = True,
    ...
    if has_header:
        ...
        if cell_xpath:
            cols = rows[0].find_elements(By.XPATH,cell_xpath)
        else:
